...
- `CONNEXIONS_INTAKE_URL` — Connexions Platform Trust Sprint intake form (already live: `https://connexions-silk.vercel.app/p/platform-trust-sprint-intake`)
- (no `PRELABZ_INTAKE_URL` — deferred under Path C; for-self triage outcomes are captured data-only by the interview agent)
...
Per Appendix B's funnel architecture, with the Path C decision applied (2026-05-18 — prelabz routing deferred until funnel data validates demand for a productisation track):
...
3. **Routing logic** (in `interview.ts`) — **Path C, v1**:
   - `for-someone-else` → POST to `CONNEXIONS_INTAKE_URL` (live: `https://connexions-silk.vercel.app/p/platform-trust-sprint-intake`) with interview payload; user is taken into the Connexions voice interview flow Dennis built 2026-05-14
   - `for-self` → write to `mcp_engagement.routing = 'data_only'`; user sees "Thanks — Dennis will follow up if it looks like a fit"; Dennis gets emailed with full interview context for manual judgment
   - `both` / `unclear` / `exploring` → same data-only path as `for-self`
4. **Email Dennis** at `NOTIFICATION_EMAIL` on every interview completion with full context, regardless of routing outcome.

Interview agent itself uses `@caistech/ai-client` internally for Claude calls — that's CAIS's own LLM cost, not the user's.

**Why Path C and not a real prelabz intake:** The PrelabzAI repo currently serves a different product (RaiseReady-branded multi-tenant founder-screening platform) — it doesn't carry a productisation-partnership surface. Building one speculatively before there's any funnel data showing real demand for a productisation track is over-investment. Once v1 of cais-au-compliance ships and the funnel starts producing for-self triage outcomes, the volume + content of those data-only captures becomes the evidence base for deciding whether to build a real prelabz productisation intake (Path A in the original decision) or collapse the triage entirely (Path B).
