...
- [ ] **Both memory layers present where relevant** — conversation memory (above) AND measured product state (the proactive/stage-aware bullet). The coach *remembers you* AND *knows your current numbers*. (A "feature to defer" only at the Gate-1 single-session validation demo; core experience for the shipped product.)
