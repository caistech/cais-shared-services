...
- [ ] **CANONICAL SENDER IDENTITY — the legal entity on every portfolio email (Spam Act pillar 2).** The *sending domain* and the *legal sender* are different things: mail goes out from the verified subdomain above, but the identification footer must name the entity behind it. **Locked 2026-07-26:**

  | Var | Value |
  |---|---|
  | `EMAIL_SENDER_NAME` | `Global Buildtech Australia Pty Ltd` |
  | `EMAIL_SENDER_EMAIL` | `dennis@corporateaisolutions.com` |
  | `EMAIL_SENDER_ABN` | `54 672 395 685` (ACN 672 395 685) |
  | `EMAIL_SENDER_POSTAL` | `76-84 Brunswick Street, Fortitude Valley QLD 4006` |
  | `EMAIL_SENDER_PHONE` | `+61402612471` |

  **Single source of truth: `portfolio-manifest.yaml` → `shared:`** — push to any project with `@caistech/portfolio-env-sync`; never retype them into a repo. They are **not secrets** (they are printed at the bottom of emails on purpose), but stay production+preview only per the Vercel env rule. `@caistech/email-compliance`'s `senderFromEnv()` reads them; **without `EMAIL_SENDER_NAME` + `EMAIL_SENDER_EMAIL` it throws**, and consumers that degrade instead will ship mail with **no identification footer** — a Spam Act gap that is invisible until someone complains.

  ⚠️ **White-label gate:** a distributor product sends under the **DISTRIBUTOR's** name and ABN, never these. Override per send with `compliance.sender` (`@caistech/email-send`). Putting our ABN on a distributor's marketing is the failure this rule exists to prevent. *(Codified 2026-07-26 after Kira shipped live email paths with the footer silently absent.)*
...
- [ ] **Jurisdiction guard — we are cleared for AUSTRALIA ONLY; email outreach to non-AU contacts is BLOCKED portfolio-wide until that country's compliance is set up.** Every other country has its own email-marketing law (US CAN-SPAM, Canada CASL, EU/UK GDPR+PECR, …); sending commercial email there before configuring it is a legal exposure. The send path calls `assertJurisdictionAllowed(recipientCountry)` (or passes `recipientCountry` to `assertCompliant`) — a non-AU contact **hard-throws** until the country is added to `SUPPORTED_OUTREACH_JURISDICTIONS` *and* its consent/identification/unsubscribe rules are implemented. Contacts must be country-tagged before a send (unknown country also blocks). **Scope is EMAIL only — LinkedIn outreach (via `@caistech/unipile-channels`) is already within platform compliance and is NOT gated.** *(Codified 2026-06-16; `@caistech/email-compliance` ≥0.2.0.)*
...
- [ ] **Regulatory inclusions declared, not assumed** — the product's legal surfaces (privacy policy, terms, voice/recording consent, content/IP acknowledgment, cookies, sender identity) each have a portfolio default, and the repo must record whether it **accepted** that default, **customised** it, or why it does **not apply**. Full ruleset, the per-inclusion defaults, and the gate: **`REGULATORY_INCLUSIONS.md`**. Two things it exists to stop: (1) **a published legal page that still contains `REPLACE`** — worse than no page, because the user has now ticked a box agreeing to a placeholder (found in production, DealFindrs 2026-05-26); (2) **assuming the operator is us** — a white-label, distributor, or founder-owned product carries THAT party's entity and ABN in every inclusion, not just the email footer (the same "whose brand travels" gate as the canonical sender identity above). Voice products additionally inherit the ElevenLabs vendor consent modal, which links a privacy policy that must therefore exist — `@caistech/elevenlabs-convai` configures no consent text, so this applies portfolio-wide. Proposed gate-readiness checks #44–#46 (`gate-readiness/criteria-PROPOSAL-regulatory.json`, awaiting ratification). *(Codified 2026-07-27.)*
...
- [ ] **Shared-change propagation — "no orphaned consumers."** When you change a shared surface (`@caistech/*` package, property-services SDK/edge contract, a shared type or Supabase schema), the change is NOT done until **every consuming repo is reconciled across code + UI + Supabase tables + verify + catalog**. A shared bump made in isolation breaks consumers (old contract → 500s), hides new data (UI never updated), or 500s writes (DB missing the column). Full standing rule + the pre-close checklist: **`cais-shared-services/CLAUDE.md` "Shared-change propagation"**. *(Codified 2026-07-10 from the property-services planning-retrieve + F2K-Checkpoint consumer change that had to move together.)*
- [ ] **Single canonical property feed — no legacy sources.** Any repo consuming AU property/site intelligence sources it ONLY via the published `@caistech/property-services-sdk` (property-services Supabase edge, X-API-Key). **No vendored copy of the SDK client, no local `supabase/functions/*-derive` edge function, no `@caistech/site-intelligence` local-GeoJSON derivation, no direct `fetch` to a legacy `/functions/v1/*-derive` endpoint.** The `check-canonical-property-feed.mjs` guard (`cais-shared-services/scripts/`, run by `feature-preflight.mjs`, exit 2) fails CI on any of these. A reviewed exception opts out with `// @canonical-feed-ok: <reason>`. This exists because "migrate to canonical" kept being declared done while sibling legacy paths (F2K-Checkpoint's 5 local `*-derive` stubs, MMC's vendored SDK fork) still shipped — the guard makes that impossible to overclaim. *(Codified 2026-07-05 from the property-services consumer audit.)*
...
- [ ] **EVERY displayed price shows its tax qualifier — "+ GST", or the equivalent for the buyer's country.** Prices are quoted **tax-exclusive**, and an unqualified figure is read by a business buyer as the amount that will leave their account — which in most jurisdictions it is not. This applies to every surface a price appears on: pricing pages, checkout copy, plan summaries, emails, FAQs, and any AI-spoken quote.
  **The label must follow the buyer, not our entity.** A product reachable from anywhere cannot hardcode "GST": it is **VAT** in the UK/EU/UAE/South Africa, **GST/HST** in Canada, **sales tax** in the US, **GST** in AU/NZ/Singapore/India. Where the product already has a currency selector, carry the tax name on the currency record and derive the suffix from it — never a literal string next to the number. Reference implementation: Kira `lib/valuation/currency.ts` (`tax` field on `Currency`, `taxSuffix()`, `formatPrice()`), where `formatMoney` stays for **valuation figures** — which are not prices and must NOT gain a tax suffix.
  ⚠️ **Do not guess whether a price is inclusive or exclusive.** If it has never been decided for a product, that is an operator decision with tax consequences, not a copy default. *(Codified 2026-07-27 after a naive-tester found $999/month on an AU product with GST stated nowhere — the rule had been given verbally more than once and was never written down, which is precisely why it kept being missed.)*
...
This is the portable checklist. The full prose (the *why*, the worked examples, the verification heuristics) is in `~/.claude/CLAUDE.md` under the named sections cited above. If this checklist and CLAUDE.md ever diverge, CLAUDE.md's prose is authoritative — re-sync this file. Companion portable docs in this repo: `BUSINESS_MODEL.md` (how the portfolio makes money), `MONETISATION_RULES.md` (the 15 monetisation rails), `THIN_MVP_RUBRIC.md` (what goes in a thin validation MVP — note its §3 "Portfolio DNA is experience" overlaps this checklist: nav chrome + Settings + voice + responsive + explanatory headers belong in even the thinnest slice), `CLIENT_HANDOVER_KIT.md` (the key/env self-serve standard for handing a product to a client / distributor / self-host operator), `SUPABASE_MIGRATION_PLAYBOOK.md` (the runbook for migrating a Supabase project's data between orgs), `VOICE_MEMORY_STANDARD.md` (the persistent voice-agent memory & state floor — rules 1–20; the rubric `/voice-auditor` loads), `REGULATORY_INCLUSIONS.md` (the legal/consent surfaces every product must carry, their portfolio defaults, and the per-repo gate that forces each to be accepted, customised or ruled out), **`GOOGLE_WORKSPACE_CONNECTOR.md`** (the canonical Drive/Gmail/Contacts setup shape — the scope set, the console steps in order, publishing-mode choice incl. the 7-day refresh-token trap in Testing, the seven silent-failure traps, and what can and cannot be automated: **one app, many consents — you do NOT create a Google Cloud project per client**), and `GATE_READINESS_CRITERIA.md` (the objective thin-MVP / Gate-1 test — catalogues every check the canonicals run, mapped to verification method + thin-MVP relevance; seeds THIN_MVP_RUBRIC v2 + the cockpit `readiness_criteria` table).
