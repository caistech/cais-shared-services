...
- [ ] **Persistent cross-session memory loop wired** — *the* point of a voice coach (one that forgets you isn't a coach). Via `@caistech/elevenlabs-convai`, end-to-end, not faked: **recall at start** (`get_conversation_context` + the `conversationContinuityPrompt` in the system prompt) → **capture/distil** mid-call (`save_memory` / `update_topic` / `save_message`) → **persist** via the workspace post-call webhook → recalled the *next* session. `migration.sql` applied to the product's OWN Supabase; `conversation_config_override` ENABLED at provision; identity **server-derived** (`resolveSession`→`auth.uid()`; tools send `conversation_id`, never `user_id`); cross-session memory is **authed-only** (anon is ephemeral — never tell an anonymous user the agent remembers them). **Storage ≠ memory: a transcript sitting on ElevenLabs (or in a DB) gives ZERO continuity until this loop is wired.**
- [ ] **Continuity is distil-then-recall, not transcript-replay** — the coach works off **distilled memories** (typed facts + per-session summary + topics), never by re-ingesting raw transcripts (they don't fit the context window and don't scale to many sessions). Transcripts are stored for history / the user to read; coaching continuity runs off the memory layer.
- [ ] **The agent PULLS the user's full state — the operator never PUSHES it.** Beyond conversation memory, the agent has **product-domain recall tools** (same `tool → webhook → product-handler → product-DB` pattern as the memory tools, identity server-derived) to fetch the user's data on demand: for Singify, the **baseline** + the **analysis result of every recording**. You never "read the baseline to the agent" via a prompt — it knows it because it can query it. **Per-session prompt overrides carry only the *just-happened* event of THIS session** (e.g. "they just finished take 3"); everything persistent (baseline, past takes + results, conversation memory) is **pulled, not re-injected**. *(Supersedes pushing persistent state via overrides; cross-session pull is authed-only / founder-hardcoded-id pre-auth.)*
- [ ] **The agent works off results, not raw media** — it can't ingest audio/video; expose the **analysis output** of each recording (scores, pitch metrics, the distilled memory), never the file. Media is stored for the user to replay; the agent knows what the analysis *found*.
