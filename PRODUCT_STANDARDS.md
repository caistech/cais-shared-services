...
- [ ] **SayFix integration** — every product has `@caistech/sayfix-embed` (or `@caistech/sayfix-client`) wired in:
  - [ ] **Widget** — `<SayFixWidget product="github-owner/repo" />` in the main layout (floating button, opens sayfix.vercel.app/new?product=xxx)
  - [ ] **Admin setup** — repo added to SayFix admin (`/admin`) with ownership type (portfolio/sister_company/client_build)
  - [ ] **Stakeholder invites** — admins can invite stakeholders via SayFix admin to let them report issues
  - [ ] **Audit gate** — product must be in SayFix repos table before release.
