...
| Package | Capability |
|---|---|
| `@caistech/property-services-sdk` | Property intelligence — **`derive`** (full `PropertyProfile`: zoning envelope, overlays, terrain/buildability, subdivision, wind/climate/BAL), **`assess`** (AI suitability), **`comparables`** (Domain AVM), **`dossier`** (ONE call = profile + assessment + AVM + panel-review checklist + write-backs — prefer over stitching), **`suggest`** (AU address autocomplete — wire to every address input), **`contribute`** (professional write-back to the parcel) + `onboard` React hook. Wraps the property-services Supabase edge functions. The substrate for the property engine. **v0.6.0+** adds dossier/suggest/contribute. ⚠️ Canonical source is `packages/property-services-sdk/`; the copy under `property-services/sdk/` is a stale, private (`do-not-publish`) archive. |
