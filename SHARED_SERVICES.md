...
| Package | Capability |
|---|---|
| `@caistech/elevenlabs-convai` | **The portfolio voice stack** — ElevenLabs Conversational AI: agent provisioning, webhook routes, persistent memory loop, anonymous sessions, and a React `VoiceWidget` (`/react`). **v0.4.1: one-conversation voice + text** — opt-in `textInput` prop renders a text box wired to `sendUserMessage` so users type/paste INTO the live session; `onReady(controls)` hands `{ sendUserMessage, sendContextualUpdate }` so the consumer can push timed/system turns the agent speaks (e.g. a wrap-up warning). Never build a parallel voice client — consume this. |
