...
export interface StartConversationParams {
  ...
  /** Resolved by the route from the verified session. Persisted on the conversation row so
   *  handleSaveMemory can derive it via getConversationBinding without a second lookup. */
...
export async function handleStartConversation(
  ...
  const { elevenlabsConversationId, elevenlabsAgentId, userId, anonSessionId, organisationId } = params;
...
export interface SaveMemoryParams {
  ...
  identity?: { userId: string; agentId?: string | null };
  /** Organisation context — every memory row must carry organisation_id. Required when `identity`
   *  is used directly (no conversation row to derive it from); falls back to the conversation's
   *  own organisation_id otherwise. */
  organisationId?: string;
}
...
export async function handleSaveMemory(
  ...
  const binding = identity
    ? { userId: identity.userId, agentId: identity.agentId ?? null, id: null, anonSessionId: null, organisationId: params.organisationId ?? null }
    : await getConversationBinding(supabase, tables, elevenlabsConversationId);
  if (!binding) {
    return { success: false, error: 'Conversation not found' };
  }

  // organisation_id is OPT-IN, not required: no shipped consumer populates it yet (nothing
  // writes it onto a conversation unless the product's resolveSession/resolveToolIdentity
  // callbacks return one). Hard-requiring it here would fail handleSaveMemory for every
  // existing consumer the moment they bump this package — the plumbing exists so a product
  // CAN scope memory by organisation once it wires resolveSession/resolveToolIdentity to
  // return organisationId; it degrades to NULL (today's behaviour) when it doesn't.
  const organisationId = binding.organisationId ?? params.organisationId ?? null;
