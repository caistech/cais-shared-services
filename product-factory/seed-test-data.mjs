 ...
 * 
 * Creates sample data for multiple products at different stages:
 * - sayfix: Stage 4 (Construction) - high readiness, validation started
 * - singify: Stage 3 (Compliance) - all fields filled
 * - deal-findrs: Stage 2 (Design & Planning) - partial fields
 * - r-and-d-tax: Stage 1 (Pre-Development) - minimal data
 */
...
function resolveCreds() {
  ...
  const env = join(ROOT, "Corporate-AI-Solutions", ".env.local");
  const url = envValue(env, "NEXT_PUBLIC_SUPABASE_URL");
  const key = envValue(env, "SUPABASE_SERVICE_ROLE_KEY");
  if (url && key) return { url, key };
  throw new Error("Could not resolve Supabase credentials");
}
...
async function seedData() {
  ...
  for (const product of testProducts) {
    const result = await rest(`product_validation_status?product_slug=eq.${product.product_slug}`, {
      method: 'POST',
      body: product,
    });
  ...
  // Also seed some lifecycle stages
  ...
  for (const stage of lifecycleStages) {
    await rest(`product_lifecycle_stage?product_slug=eq.${stage.product_slug}`, {
      method: 'POST',
      body: stage,
    });
  ...
  // Seed one certificate
  await rest(`certificate_of_occupancy?product_slug=eq.sayfix`, {
    method: 'POST',
    body: {
      product_slug: 'sayfix',
  ...
  for (const sensor of sensorData) {
    await rest('smart_sensors', { method: 'POST', body: sensor });
  }
  console.log(`  └─ Smart Sensors: Seeded ${sensorData.length} readings`);

  console.log('\n✅ Test data seeded successfully!');
  console.log('\nRefresh /admin/pipeline/factory to see:');
  console.log('- sayfix in Stage 4 (Construction) with Certificate VALID');
  console.log('- singify in Stage 3 (Compliance) with Sensor warnings');
  console.log('- deal-findrs in Stage 2 (Design & Planning)');
  console.log('- r-and-d-tax in Stage 1 (Pre-Development)');
}
