...
**Separate task (Corporate-AI-Solutions repo — NOT this session):** the CAS cockpit `/api/methodology/sync` handler must, after recording the methodology evidence, PATCH InvestorPilot (by `ip_campaign_id` + `ref`) to mark the partner interviewed — the relay that replaces the legacy direct feed. This must exist before (or with) the legacy retirement above. Flag if not already built.
