...
**Backfill:** running every existing product through this pipeline (letting interview evidence produce each go/no-go) *is* the proper rerun of the old heuristic distributor sweep. **Status (2026-05-27):** the stage-3 *voice loop* — the Connexions interview → structured-feedback **return leg** — **is operational** (wired + env-live in prod, proven for the platform-trust pilot, generalized per-product via `POST /api/methodology/panels` + `forwardMethodologyIntake`). The backfill is gated not on the voice loop but on the stage-3 *front leg*: auto-creating BOTH IP streams from the idea card (`HARNESS_BUILD_WORKLIST.md` #4) + the cockpit response rollup. See `STAGE3_RETURN_LEG_HANDOFF.md` + memory `project_stage3_ip_connexions_wiring`.
