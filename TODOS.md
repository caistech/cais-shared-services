...
## 4. Portfolio sweep — `@supabase/ssr` middleware logout-on-refresh fix ✅ DONE (2026-06-18)

- **STATUS (2026-06-18): COMPLETE.** All 5 actually-buggy repos fixed + deployed green. Pushed to `main` (live in prod): DealFindrs `f9aee80`, F2K-OffshoreModular `74c1d44`, LongtailAIVentureStudio `f4b67ef`. Merged via PR to `main`: Corporate-AI-Solutions (PR #27), Tenderwatch (PR #2). The 15 already-safe + 3 N/A repos were left untouched per the triage below. Two incidental deploy blockers fixed en route (NOT caused by this change, surfaced by the redeploys): (a) Longtail's Vercel project was missing the `@caistech` GitHub-Packages registry token → `NODE_AUTH_TOKEN` + `GITHUB_PACKAGES_TOKEN` now set (sensitive, prod+preview; see the `reference_caistech_vercel_registry_token` memory), and it carried an orphaned `react-compiler` ESLint rule with no plugin installed → removed in `f4b67ef`; (b) F2K-Checkpoint's `chore/canonical-resend-email` branch had a stale pnpm lockfile for `@caistech/email-compliance@^0.2.0` → regenerated (`1e92d88`, ERROR→READY).
