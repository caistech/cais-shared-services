...
- ~~**Kira + LaunchReady NOT auditable — canonical deploys 401**~~ **← WRONG, corrected 2026-05-27 (alias-mismatch artifact).** F2K Checkpoint, Kira, and LaunchReady are all **publicly reachable** on their real production aliases — `f2k-checkpoint-new.vercel.app`, `kira-rho.vercel.app`, `launchready-ruby.vercel.app` (all `200`, real product landings). The `401`s were only on the **`<project>-corporate-ai-solutions.vercel.app`** team-suffixed aliases, which Vercel auto-protects with deployment protection — and that's exactly what the **portfolio-manifest `canonical_url` points at**, so the audit + my first checks probed the protected alias. **ROOT CAUSE TO FIX: the manifest's `canonical_url` fields point at the protected team aliases — repoint them at the public `<project>-<word>` aliases** so research links + future audits hit the reachable URL. This is a manifest-URL correction, NOT a "lift protection" task. (Lesson: probe the project's real production alias from `get_project.domains`, not the manifest URL or the `-<team>` suffix.)
...
- **#15 F2K Checkpoint 401 → CLEARED (2026-05-27).** Real alias `f2k-checkpoint-new.vercel.app` returns `200` with a public landing + "Sign In"; the `401` was the team-suffixed alias (or since-lifted protection), not the product. No longer a blocker — drop from the parked list. (Same alias-mismatch as Kira/LaunchReady above.)
...
|---|---------|-------------|---------|-----|
| 15 | **F2K Checkpoint** | ✅ `F2K-Checkpoint` | ~~Canonical alias returns **401**~~ → **CLEARED 2026-05-27**: real alias `f2k-checkpoint-new.vercel.app` is `200`/public. The 401 was the protected `-corporate-ai-solutions` team alias (manifest pointed there). | Repoint manifest `canonical_url` → the public alias. (Kira + LaunchReady: same correction — `kira-rho` / `launchready-ruby` are `200`.) |
