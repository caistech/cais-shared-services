...
| partner-pilot | 💰 | ✓ | ? | ? | ? | ? | ? | ? | ? | ? | Auth flow paused 2026-05-16. RC1 ✓ 2026-05-20. RC1 closed: `src/app/layout.tsx` metadata customized (Shape D-app — title + OpenGraph + Twitter); `.npmrc` aligned to `NODE_AUTH_TOKEN`; Vercel `NODE_AUTH_TOKEN` aliased to `GITHUB_PACKAGES_TOKEN` (production + preview); PLATFORMS url switched to `partner-pilot-corporate-ai-solutions.vercel.app`. PartnerPilot commits ab007a5/fc16ecb/cd727d8. |
...
| connexions | ⚪ | ✓ | ✓ | ? | ? | ? | ? | ✓ | ? | ? | **Hot (5 RCs)**. RC7 ✓ + RC2 ✓ + RC1 ✓ 2026-05-20. RC1 closed: config/client.ts repointed from "f27 survey" → canonical Connexions (commit 25d631a); Vercel NODE_AUTH_TOKEN aliased to GITHUB_PACKAGES_TOKEN (deploy was failing on @caistech 401); PLATFORMS url switched to connexions-corporate-ai-solutions.vercel.app (CAS commit de53745). Widget-consumer with demo + generic agent env (no router leak). |
...
| 2026-05-20 | Batch 2 follow-up — 3 immediate URL scrubs landed in Corporate-AI-Solutions/src/lib/constants.ts using already-working team aliases: mmcbuild → mmcbuild-webapp.vercel.app, mova → mova-pi.vercel.app, omq-outreach → omq.vercel.app. Verified via probe. (commit 43085d0) | claude (this session) |
| 2026-05-20 | Batch 2 follow-up — ssoProtection.deploymentType patched from "all_except_custom_domains" → "preview" on 4 projects via Vercel API: connexions, ndissda-automate, partner-pilot, gbta-openclaw. Production URLs now publicly accessible (preview deploys remain protected). BUT exposed that the underlying deploys themselves are broken/wrong content (connexions builds as "f27 survey" — see config/client.ts platform.name; ndissda-automate serves "PF Platform"; partner-pilot is default Create-Next-App; gbta-openclaw has empty title). Real fix is per-repo content remediation, not constants.ts URL changes. leadspark-tenant project does not exist on the team. |
| 2026-05-20 | Connexions RC1 closed (Shape A from PORTFOLIO_FIX_BRIEF). Fix: (1) config/client.ts platform.name "f27 survey" → "Connexions", company "key2factoiry" → "Corporate AI Solutions", parent.apiUrl → canonical (Connexions commit 25d631a). (2) Vercel deploy was failing 401 on @caistech package fetch — root cause: .npmrc reads ${NODE_AUTH_TOKEN} but only GITHUB_PACKAGES_TOKEN was set in Vercel env (CI-side rename in 805a292 never propagated to Vercel). Aliased NODE_AUTH_TOKEN to the same PAT for production/preview/development via vercel CLI, redeployed (commit 2b010d9). (3) Canonical URL connexions-corporate-ai-solutions.vercel.app now serves &lt;title&gt;Connexions&lt;/title&gt; + correct description. (4) PLATFORMS entry switched from hijacked connexions-silk to canonical alias, HIJACKED comment removed (CAS commit de53745). | claude (this session) |
| 2026-05-20 | **gbta-openclaw / EasyOpenClaw — product killed.** Operator decision: retire the product entirely. Removed easyopenclaw entry from Corporate-AI-Solutions/src/lib/constants.ts PLATFORMS (CAS commit 75fe3fa, replaces unpushed 036b5a1). Removed gbta-openclaw row from this tracker. GitHub repo (dennissolver/gbta-openclaw) and Vercel project (gbta-openclaw on team_hwN7IFtd2Fo3DCj9C67ZwI1t) remain alive as orphans per operator choice. Earlier today's repo-side title fix (gbta-openclaw commit c71cfe9 — added &lt;Head&gt; to apps/frontend/pages/_app.jsx) lands as orphaned work. Open subthreads dropped: Vercel Attack Challenge on canonical URL (no longer needs resolution). | claude (this session) |
| 2026-05-20 | **r-and-d-tax RC1 closed.** Same ssoProtection patch applied as the 4-repo batch above: deploymentType `all_except_custom_domains` → `preview` on project `prj_6fiASaGSs4EnvSha26AY4Q7luzDt` (r-and-d-tax-eligibility-work-recording). Canonical URL `r-and-d-tax-eligibility-work-recording-corporate-ai-solutions.vercel.app` now returns 200 + serves real product (title: *R&amp;D Tax Tracker — Automated R&amp;D Tax Incentive Recording*). Also fixed truncated URL in `Corporate-AI-Solutions/src/lib/constants.ts:383` (`...work-record` → `...work-recording-corporate-ai-solutions`). Patch script: `scripts/patch-ssoprotection.py` — re-usable for any future projects hitting the same RC1 pattern. Required a fresh SAML-scoped Vercel token (existing `~/.vercel-token` had expired SAML session). | claude (this session) |
