 ...
 *   3. manifest.projects[].inherit_shared â†’ PLATFORM_TRUST_SUPABASE_URL
 *      (extracts ref from the URL)
 *   4. .env.local NEXT_PUBLIC_SUPABASE_URL (auto-discovered from repo root)
 *   5. Skip (notify user)
 *
 * Env / tokens:
 *   SUPABASE_MANAGEMENT_TOKEN â€” Management API token (falls back to
...
// ---------------------------------------------------------------------------
// Supabase Management API â€” fetch service_role key from project ref
// ---------------------------------------------------------------------------
...
function loadTestAccountsConfig(repoRoot) {
  const p = join(repoRoot, "test-accounts.config.json");
...
// ---------------------------------------------------------------------------
// Supabase Admin API â€” user operations
// ---------------------------------------------------------------------------
...
function writeEnvVars(envPath, vars) {
  ...
  console.log(`  âœ“ wrote ${Object.keys(vars).length} var(s) to ${envPath}`);
}
...
async function pushToVercel(vercelProjectId, vars) {
  ...
  if (!token) {
    console.log("  âš  VERCEL_TOKEN not set â€” skipping Vercel push");
    return;
  }
  ...
  for (const [key, value] of Object.entries(vars)) {
    ...
    if (!res.ok) {
      const body = await res.text().catch(() => "");
      if (res.status === 409) {
        console.log(`  âš  ${key} already exists on Vercel (HTTP 409) â€” update via dashboard or --overwrite`);
      } else {
        console.log(`  âš  Vercel env push ${key}: HTTP ${res.status} â€” ${body.slice(0, 100)}`);
      }
    } else {
      console.log(`  âœ“ pushed ${key} to Vercel (production+preview, sensitive)`);
    }
  }
}
...
async function main() {
  ...
  for (const slug of slugs) {
    ...
    if (!supabaseRef) {
      console.log(`  SKIPPED â€” could not determine Supabase project ref.`);
    ...
    if (!hasRepoRoot) {
      console.log(`  âš  Repo root not found at ${repoRoot} â€” .env.local write skipped`);
    }
    ...
    for (const admin of adminUsers) {
      ...
      if (args.dryRun) {
        console.log(`  â†’ would create admin: ${email}`);
        continue;
      }
      try {
        ...
        if (result.skipped) {
          console.log(`  âœ“ ${email} â€” already exists (skipped)`);
          totalSkipped++;
        } else if (result.created === false) {
          console.log(`  âœ“ ${email} â€” already exists, password reset`);
          totalCreated++;
        } else {
          console.log(`  âœ“ ${email} â€” created`);
          totalCreated++;
        }
      } catch (e) {
        console.error(`  âœ— ${email} â€” ${e.message}`);
        totalErrors++;
      }
    }
...
// QA agent identities from the product's OWN config (one source of truth per deployment).
    ...
    for (const agent of agents) {
      ...
      try {
        ...
        if (result.skipped) {
          ...
          console.log(`  âœ“ ${agent.email} â€” ${result.created === false ? "password reset" : "created"}`);
          totalCreated++;
          if (hasRepoRoot) {
            writeEnvVars(join(repoRoot, ".env.local"), { [agent.emailVar]: agent.email, [agent.pwVar]: pw });
          }
          if (args.vercel) {
            const proj = projects[slug];
            if (proj?.vercel_project_id) {
              await pushToVercel(proj.vercel_project_id, { [agent.emailVar]: agent.email, [agent.pwVar]: pw });
            } else {
              console.log(`  âš  No vercel_project_id for ${slug} â€” Vercel push skipped`);
            }
          }
        }
      } catch (e) {
        console.error(`  âœ— ${agent.email} â€” ${e.message}`);
        totalErrors++;
      }
    }
  ...
  if (args.dryRun) console.log(`\n  DRY RUN â€” no changes persisted. Re-run without --dry-run to apply.`);
}
