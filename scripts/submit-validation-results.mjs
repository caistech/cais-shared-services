 ...
 *
 * The script:
 *   - Writes summary columns to product_validation_status (test_part_a/b/c/d, findings, etc.)
 *   - Writes per-check granular results to readiness_results via recordReadiness()
 *
 * Usage:
 *   node scripts/submit-validation-results.mjs <slug> --file results.json
 ...
 *   {
 *     "parts": {
 *       "a_admin_portal":   { "status": "passed|warning|failed", "checks": { "check_code": { "status": "passed|fail|na", "note": "..." }, ... } },
 *       "b_user_portal":    { "status": "passed|warning|failed", "checks": { ... } },
 *       "c_auth_flows":     { "status": "passed|warning|failed", "checks": { ... } },
 *       "d_scaffold":       { "status": "passed|warning|failed", "checks": { ... } }
 *     },
 *     "findings": ["string", ...],
...
export async function main() {
  ...
  // Check if row exists (parse JSON for GET)
  let existingId = null;
  try {
    ...
    // Table might not have row yet — that's fine, we'll INSERT
  }
  ...
  console.log(`✅ Summary written to product_validation_status for ${slug}`);

  // Step 2: Optionally write per-check granular results to readiness_results
  // Maps validation test check codes → VT_ prefixed readiness_criteria codes
  const CHECK_TO_VT = {
    // Part A: Admin Portal
  ...
  if (has("record-readiness")) {
    ...
    for (const [partKey, part] of Object.entries(results.parts || {})) {
      ...
      for (const [checkCode, check] of Object.entries(part.checks)) {
        ...
        if (!vtCode) continue; // skip unmapped codes
        const checkStatus = check.status === "passed" ? "pass" : check.status === "failed" || check.status === "fail" ? "fail" : "pass";
    ...
    if (readinessChecks.length > 0) {
      try {
        ...
        console.log(`✅ ${readinessChecks.length} per-check results written to readiness_results`);
      } catch (e) {
        console.error(`⚠️ record-readiness failed (non-critical): ${e.message}`);
      }
    }
  }
