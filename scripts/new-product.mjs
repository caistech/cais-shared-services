 ...
 * template so ANY new product can be spun up — no clone-RaiseReady coupling.
 *
 * Decisions baked in (see NEW_PRODUCT_CREATOR_SPEC.md §1):
 ...
 *     sensitive, production+preview only — never development).
 ...
 *   - IDE: no automation — prints the local path.
 *
 * Usage:
 ...
 * that wrote `KEY="sk-..."` (AIFTIS-Demo did) must NOT propagate the quotes —
...
function preflight() {
  ...
  CONFIG.description ??= `${CONFIG.displayName} — Corporate AI Solutions`;
  ...
  if (CONFIG.voice && !process.env.ELEVENLABS_API_KEY) {
    warn("ELEVENLABS_API_KEY unset — voice step will be skipped (run with --no-voice to silence).");
    CONFIG.voice = false;
  }
...
async function stepSupabase() {
  ...
  if (!ref) {
    const pw = randomBytes(24).toString("base64url"); // URL-safe → pooler-string safe
    ...
    if (DRY) ref = "<ref>";
      ...
      // Resolve the ref from the dashboard URL — the ORG ID is also 20 chars, so never
  ...
  if (DRY) { info("DRY: poll ACTIVE_HEALTHY → get keys → link → db push (pooler) → verify seed"); return ref; }
  ...
  ok(`keys written to .env.local (anon ${anon ? "✓" : "✗"}, service_role ${svc ? "✓" : "✗"})`);
  ...
  if (existsSync(join(repoDir, "supabase", "migrations"))) {
    ...
    if (bad(pushed)) warn("db push may have failed — check `supabase db push` manually");
    else ok("migrations applied via session pooler");
  } else {
    warn("no supabase/migrations in the repo — skipping db push (template should ship the DNA migration)");
  }
  return ref;
}
...
async function stepVercel() {
  ...
  // (A PATCH cannot convert encrypted→sensitive, so secrets are delete+recreate.)
  ...
  for (const key of wanted) {
    ...
    if (!value) { warn(`${key} not in .env.local — skipped`); continue; }
...
async function stepDeploy(prjId) {
  ...
  if (!prjId) { warn("no Vercel project id — skipping deploy"); return; }
  const teamQ = `?teamId=${CONFIG.vercelTeam}`;
  ...
  if (!repoId) { warn("project not git-linked (no repoId) — skipping deploy; merge to main to deploy"); return; }
...
function stepHarvestLocal() {
  step("Harvest shared CAIS keys → .env.local");
...
// ---------------------------------------------------------------------------
// step: Resend — mint a PER-PRODUCT sending key + verify a test send
// ---------------------------------------------------------------------------
...
// from a full-access CAIS admin key — NOT the shared portfolio key. And we
...
async function stepResend() {
  ...
  if (!admin) {
    warn("RESEND_ADMIN_API_KEY (full-access) not found — keeping the harvested shared RESEND_API_KEY.");
  ...
  // (the token is shown only at creation, so we can't re-read it — don't churn a new one).
  ...
  if (already && getEnv("RESEND_API_KEY")) {
    ok(`per-product key '${SLUG}' exists (id ${already.id}) — keeping current RESEND_API_KEY`);
  } else {
    ...
    if (!dom) { warn(`Resend domain '${DOMAIN}' not on this account — cannot mint a scoped key. Keeping shared key.`); return; }
    ...
    if (!created.ok || !cj.token) { warn(`mint failed (${created.status}): ${JSON.stringify(cj).slice(0, 120)} — keeping shared key`); return; }
    setEnv("RESEND_API_KEY", cj.token);
    ok(`minted per-product Resend key '${SLUG}' (id ${cj.id}), scoped to ${DOMAIN}`);
  }

  // Verify a real send — catches a broken sender BEFORE the first user does.
  ...
  if (sent.ok && sj.id) ok(`verification email sent (id ${sj.id}) to ${to} — sender works`);
  else warn(`test send failed (${sent.status}): ${JSON.stringify(sj).slice(0, 160)} — fix the sender before real users`);
}

// ---------------------------------------------------------------------------
// step: onboard (auth + SMTP + email templates + manifest) — reuse the script
// ---------------------------------------------------------------------------

function stepOnboard(ref) {
  if (CONFIG.skipPortfolio) { step("Onboard — skipped (--skip-portfolio)"); return; }
  step("Onboard (Supabase Auth + SMTP + email templates + manifest)");
  const script = join(HUB_ROOT, "scripts", "onboard-new-project.sh");
  if (!existsSync(script)) { warn("onboard-new-project.sh not found — skipping"); return; }
  const env = {
    ...process.env,
    VERCEL_TOKEN,
...
// ---------------------------------------------------------------------------
// step: ElevenLabs (shared helper only — never the deprecated webhook shape)
// ---------------------------------------------------------------------------

async function stepVoice() {
  if (!CONFIG.voice) { step("ElevenLabs voice — skipped (no key / --no-voice)"); return; }
  ...
  if (typeof provision !== "function") { warn("provisionVoiceAgent not exported by the hub — skipping."); return; }
  ...
  try {
    ...
    if (!agentId) { warn("provision returned no agentId — check the hub package."); return; }
...
function stepRegisterAndGit(ref) {
  ...
  const repoDir = join(PORTFOLIO_BASE, SLUG);
  // working branch off main; commit any local changes; push — STOP before PR.
